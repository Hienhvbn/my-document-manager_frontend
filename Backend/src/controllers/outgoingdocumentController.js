...
const createOutgoingdocument = async (req, res) => {
	try {
		const formData = req.body; // Lấy dữ liệu từ request
		const newDocument = new OutgoingDocument(formData); // Tạo bản ghi mới
...
const getOutgoingdocument = async (req, res) => {
  try {
	const documents = await OutgoingDocument.find({}).select();
...
const deleteOutgoingdocument = async(req, res) => {
  try {
	...
	const deletedDocuments = await OutgoingDocument.deleteMany({ _id: { $in: _id } });
...
const editOutgoingdocument = async(req, res) => {
  try {
	const { id } = req.params;
	const updateData = req.body;

	const updatedDocument = await OutgoingDocument.findByIdAndUpdate(id, updateData, { new: true });
  ...
  module.exports = { createOutgoingdocument, getOutgoingdocument, deleteOutgoingdocument, editOutgoingdocument };
